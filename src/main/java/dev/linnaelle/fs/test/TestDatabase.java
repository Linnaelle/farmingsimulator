...
public class TestDatabase {
    ...
    private static void testDifficulte() {
        try (Connection conn = DatabaseManager.get();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT COUNT(*) as count FROM Difficulte")) {

            if (rs.next()) {
                int count = rs.getInt("count");
                System.out.println("[DIFFICULTES] " + count + " difficultes trouvees");

                ResultSet examples = stmt.executeQuery("SELECT nom, goldDepart, multiplicateurAchat, multiplicateurVente FROM Difficulte LIMIT 5");
                while (examples.next()) {
                    System.out.println("   - " + examples.getString("nom") +
                                     " (Gold de départ: " + examples.getDouble("goldDepart") +
                                     ", Multiplicateur prix achat: " + examples.getDouble("multiplicateurAchat") +
                                     ", Multiplicateur prix vente: " + examples.getDouble("multiplicateurVente") + ")");
                }
                examples.close();
            }
        } catch (Exception e) {
            System.err.println("[ERROR] Erreur test Difficulte: " + e.getMessage());
        }
    }
}